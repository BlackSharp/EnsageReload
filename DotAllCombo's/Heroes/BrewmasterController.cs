...
namespace DotaAllCombo.Heroes
{
    ...
    internal class BrewmasterController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
                ...
                if (Me.CanCast())
                {
                    ...
                    if (Me.Distance2D(_mousepos) <= 1200)
                    {
                        ...
                        if (_w != null && Menu.Item("Skills: ").GetValue<AbilityToggler>().IsEnabled(_w.Name) &&
                            _w.CanBeCasted() && E.Position.Distance2D(Me.Position) < 850 && E.Position.Distance2D(Me.Position) > 300 && _r.CanBeCasted() && Utils.SleepCheck("W"))
                        {
                            _w.UseAbility(E);
                            Utils.Sleep(150, "W");
                        }
                        ...
                        if (_w != null && Menu.Item("Skills: ").GetValue<AbilityToggler>().IsEnabled(_w.Name) &&
                            ...
                            !E.HasModifier("modifier_brewmaster_drunken_haze") &&
                        ...
                        //orchid
                        if (_orchid != null && Menu.Item("Items")
            ...
            var targets =
                ObjectManager.GetEntities<Hero>()
                    .Where(
                        enemy =>
                            enemy.Team == Me.GetEnemyTeam() && !enemy.IsIllusion() && enemy.IsVisible &&
                            enemy.IsAlive &&
                            enemy.Health > 0)
                    .ToList();
                    ...
                    (Game.MousePosition.Distance2D(E) <= 1000 || E.Distance2D(Me) <= 600))
                {
                    ...
                    var necronomicons =
                        ObjectManager.GetEntities<Creep>()
                            .Where(x => x.ClassId == ClassId.CDOTA_BaseNPC_Creep
                    ...
                    {
                        if (!(E.Position.Distance2D(v.Position) < 1550) ||
                            !Utils.SleepCheck(v.Handle.ToString())) continue;
                        v.Attack(E);
                        Utils.Sleep(700, v.Handle.ToString());
                    }
                    //Illusions	
                    ...
                    foreach (var illusion in illus.TakeWhile(illusion => Utils.SleepCheck("illu")))
                    {
                        illusion.Attack(E);
        ...
        public void OnLoadEvent()
        {
            ...
            _menuSkills.AddItem(new MenuItem("Skills: ", "Skills: ").SetValue(new AbilityToggler(
