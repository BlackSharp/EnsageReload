    ...
    internal class TerrorbladeController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            var s = ObjectManager.GetEntities<Hero>()
                .Where(x => x.IsVisible && x.IsAlive && x.Team != Me.Team && !x.IsIllusion && x.Distance2D(Me) <= 1500)
                .ToList();
            if (Active)
            {
                ...
                if (E.IsAlive && !E.IsInvul())
                {
                    if (Me.IsAlive && Me.Distance2D(E) <= 1900)
                    {
                        ...
                        _sheep = E.ClassId == ClassId.CDOTA_Unit_Hero_Tidehunter
                            ? null
                            : Me.FindItem("item_sheepstick");
                        ...
                        if (_manta != null && Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_manta.Name) &&
                            _manta.CanBeCasted() && E.Position.Distance2D(Me.Position) <=
                            Me.GetAttackRange() + Me.HullRadius
                            && (_e == null || !_e.CanBeCasted() ||
                                !Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_e.Name))
                        ...
                        if ( // Shiva Item
                            ...
                            && !E.IsMagicImmune() && _shiva.CanBeCasted() && Utils.SleepCheck("shiva") && Me.Distance2D(E) <= 600
                        )
                        {
                            _shiva.UseAbility();
                    ...
                    var illu = ObjectManager.GetEntities<Unit>()
                        .Where(x => x.ClassId == ClassId.CDOTA_Unit_Hero_Terrorblade && x.IsIllusion
                    ...
                    if (illu.Count > 0)
                    {
                        foreach (var v in illu)
                        {
                            if (
                                ...
                                && v.NetworkActivity != NetworkActivity.Attack && v.CanAttack() &&
            ...
            if (Menu.Item("ult").GetValue<bool>())
            {
                ...
                var ult = ObjectManager.GetEntities<Hero>()
                    .Where(x => x.IsVisible
                                ...
                                && x.Team != Me.Team
                                && !x.IsIllusion
                                && x.Distance2D(Me) <= _r.GetCastRange() + Me.HullRadius + 24)
                    .ToList()
                    .OrderBy(x => (double) x.MaximumHealth / x.Health)
                    .FirstOrDefault();
                ...
                if (ult != null && Menu.Item("ultEnem").GetValue<bool>())
                {
                    ...
                    if (!linkens
                        ...
                        && Me.Health < Me.MaximumHealth / 100 * Menu.Item("healme").GetValue<Slider>().Value &&
                        Utils.SleepCheck("R"))
                    {
                        _r.UseAbility(ult);
                        Utils.Sleep(500, "R");
                ...
                if (ult == null ||
                    ult.Health < ult.MaximumHealth / 100 * Menu.Item("heal").GetValue<Slider>().Value ||
                    illu == null || illu.Health <
                    illu.MaximumHealth / 100 * Menu.Item("heal").GetValue<Slider>().Value)
                {
        ...
        public void OnLoadEvent()
        {
            ...
            Menu.AddItem(
                new MenuItem("ultAlly", "Use Ult in Ally if is no suitable enemy and Illusion").SetValue(false));
