        ...
        public void Combo()
        {
            ...
            if (Push)
            {
                ...
                var remnant = ObjectManager.GetEntities<Unit>()
                    .Where(unit => unit.IsValid && unit.IsAlive && unit.Team == Me.Team &&
                                   unit.Name == "npc_dota_ember_spirit_remnant")
                    .ToList();

                if (fount != null)
                {
                    var angle = Me.FindAngleBetween(fount.Position, true);
                    var pos = new Vector3((float) (Me.Position.X + _r.GetCastRange() * Math.Cos(angle)),
                    ...
                    if (remnant.Count(x => x.Distance2D(Me) <= 10000) > 0)
                    {
                        ...
                        {
                            for (var i = 0; i < remnant.Count; ++i)
                            {
                                var kill =
                                    remnant[i]
                                        .Modifiers.Where(y => y.Name == "modifier_ember_spirit_fire_remnant_thinker")
                                        .DefaultIfEmpty(null)
                                        .FirstOrDefault();
            ...
            */
                ...
                var remnant = ObjectManager.GetEntities<Unit>()
                    .Where(unit => unit.Name == "npc_dota_ember_spirit_remnant")
                    .ToList();
