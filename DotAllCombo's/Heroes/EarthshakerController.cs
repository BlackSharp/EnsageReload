    ...
    internal class EarthshakerController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            if (Active && Me.IsAlive && Utils.SleepCheck("activated"))
            {
                ...
                {
                    if (Me.HasModifier("modifier_earthshaker_enchant_totem") && !Me.IsAttacking() &&
                        Me.Distance2D(E) <= 300 && Utils.SleepCheck("WMod"))
                    {
                        Me.Attack(E);
                            ...
                            !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_vail.Name))
                        {
                                ...
                                !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_ethereal.Name))
                            {
                                ...
                                if ( // Dagon
                                    Me.CanCast()
                                    && _dagon != null
                                    && (_ethereal == null || modifEther || _ethereal.Cooldown < 18)
                                    && !E.IsLinkensProtected()
                                    && _dagon.CanBeCasted()
                                    && Me.Distance2D(E) <= 1400
                                    && !E.IsMagicImmune()
                    ...
                    if (Menu.Item("orbwalk").GetValue<bool>() && Me.Distance2D(E) <= 1600 &&
                        !Me.HasModifier("modifier_earthshaker_enchant_totem"))
                    {
        ...
        private double GetDamageTaken(Hero victim, ref List<Hero> enemies)
        {
            ...
            var creeps = ObjectManager.GetEntities<Unit>()
                .Where(x =>
                    ...
                    victim.Distance2D(x) <= _r.GetCastRange() + Me.HullRadius + 24 &&
                    x.IsSpawned && x.IsValidTarget())
                .ToList();
            var creepsECount = creeps.Count;
            ...
            dmgResult = _r != null && Menu.Item("AutoSpells").GetValue<AbilityToggler>().IsEnabled(_r.Name) &&
                ...
                ? creepsECount * _creepsDmg[_r.Level] + _rDmg[_r.Level] + _eDmg[_e.Level]
                : 0;
            ...
            if (victim.NetworkName == "CDOTA_Unit_Hero_Spectre" && victim.Spellbook.Spell3.Level > 0)
            {
                dmgResult = _r != null && Menu.Item("AutoSpells").GetValue<AbilityToggler>().IsEnabled(_r.Name) &&
                            ...
                            Menu.Item("Heel").GetValue<Slider>().Value
                    ? creepsECount * _creepsDmg[_r.Level] + _rDmg[_r.Level] + _eDmg[_e.Level] *
                      (1 - (0.10 + victim.Spellbook.Spell3.Level * 0.04))
                    : 0;
            ...
            {
                dmgResult +=
                (Me.MaximumDamage + Me.MinimumDamage) / 2 * (_wDmg[_w.Level] / 100) + Me.BonusDamage -
                victim.DamageResist + _eDmg[_e.Level] * (1 - victim.MagicDamageResist);
            }
            ...
            if (Me.HasModifier("modifier_earthshaker_enchant_totem")
                ...
                && Menu.Item("AutoSpells").GetValue<AbilityToggler>().IsEnabled(_w.Name))
            {
                dmgResult += (Me.MaximumDamage + Me.MinimumDamage) / 2 + Me.BonusDamage - victim.DamageResist +
                             _eDmg[_e.Level] * (1 - victim.MagicDamageResist);
            }
        ...
        private void AutoSpells()
        {
            _enemies = ObjectManager.GetEntities<Hero>()
                .Where(x => x.IsVisible && x.IsAlive && x.Team != Me.Team && !x.IsMagicImmune() && !x.IsIllusion)
                .ToList();
            ...
            if (Menu.Item("autoUlt").IsActive())
            {
                ...
                foreach (var v in _enemies.Where(x => !x.IsMagicImmune()))
                {
                    ...
                    if (
                        _enemies.Count(
                            x => x.Health <= _damage[v.Handle] &&
                                 v.Distance2D(x) <= _r.GetCastRange() + Me.HullRadius + 24) >=
                        Menu.Item("Heel").GetValue<Slider>().Value)
                    {
                        uint elsecount = 0;
                        elsecount += 1;
                        ...
                        else if (_w != null && _w.CanBeCasted() && Me.Distance2D(v) <= 900 + Me.HullRadius + 24 &&
                                 Me.AghanimState()
                                 && Menu.Item("AutoSpells").GetValue<AbilityToggler>().IsEnabled(_w.Name)
                    ...
                    )
                    {
                        ...
                        Utils.Sleep(250, "blink");
                                 ...
                                 x => Me.Distance2D(x) <= 300 + Me.HullRadius + 24 && v.Health <= _damage[v.Handle]) >=
                             1)
                    {
                        if (_r == null || !_r.CanBeCasted() ||
                            !Menu.Item("AutoSpells").GetValue<AbilityToggler>().IsEnabled(_r.Name) ||
                            _r.CanBeCasted() && _enemies.Count(
                                x => x.Health <= _damage[v.Handle] &&
                                     Me.Distance2D(x) <= _r.GetCastRange() + Me.HullRadius + 24) <=
                            Menu.Item("Heel").GetValue<Slider>().Value)
                        {
                            if (Me.Distance2D(v) <= 500 + Me.HullRadius + 24 &&
                                Me.Distance2D(v) >= 300 + Me.HullRadius + 24 && Utils.SleepCheck("Move"))
                            {
                                Me.Move(v.Position);
                            ...
                            if (v.Distance2D(Me) <= 300 + Me.HullRadius + 24)
                            {
                                         ...
                                         !Me.HasModifier("modifier_earthshaker_enchant_totem") && Utils.SleepCheck("W"))
                                {
                                    ...
                                    Utils.Sleep(250, "W");
                                         ...
                                         && Menu.Item("AutoSpells").GetValue<AbilityToggler>().IsEnabled(_w.Name)
                                         && v.Health <=
                                         (Me.MaximumDamage + Me.MinimumDamage) / 2 * (_wDmg[_w.Level] / 100) +
                                         Me.BonusDamage + _eDmg[_e.Level])
                                {
                                    if (Me.AghanimState())
                                    {
                                        if (_w != null && _w.CanBeCasted()
