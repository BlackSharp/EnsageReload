    ...
    internal class BroodmotherController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            if (_lastHitkey && !_combokey && !_chasekey && Utils.SleepCheck("combo") && !Game.IsPaused)
            {
                ...
                var enemies = ObjectManager.GetEntities<Hero>()
                    .Where(hero => hero.IsAlive && !hero.IsIllusion && hero.IsVisible && hero.Team != Me.Team)
                    .ToList();
                ...
                var creeps = ObjectManager.GetEntities<Creep>()
                    .Where(creep => (creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane ||
                                     ...
                                     creep.ClassId == ClassId.CDOTA_Unit_VisageFamiliar && creep.Team != Me.Team ||
                                     creep.ClassId == ClassId.CDOTA_Unit_SpiritBear && creep.Team != Me.Team ||
                                     creep.ClassId == ClassId.CDOTA_BaseNPC_Invoker_Forged_Spirit &&
                                     creep.Team != Me.Team || creep.ClassId == ClassId.CDOTA_BaseNPC_Creep &&
                                     creep.IsAlive && creep.IsVisible && creep.IsSpawned) && creep.Health <= 259)
                    .ToList();

                var creepQ = ObjectManager.GetEntities<Creep>()
                    .Where(creep => creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane ||
                                    creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege ||
                                    creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral ||
                ...
                var list = ObjectManager.GetEntities<Unit>()
                    .Where(spiderlings => spiderlings.ClassId == ClassId.CDOTA_Unit_Broodmother_Spiderling)
                    .ToList();
                ...
                if (_useQ && _q.CanBeCasted() && Me.IsAlive)
                    ...
                    {
                        if (creep.Health <= Math.Floor(_spiderQ[spiderlingsLevel] * (1 - creep.MagicDamageResist)) &&
                            creep.Health > 45 && creep.Team != Me.Team)
                        {
                    ...
                    for (var c = 0; c < countcreep; c++)
                    {
                        for (var s = 0; s < count; s++)
                        {
                            if (creeps[c].Position.Distance2D(list[s].Position) <= 500 &&
                                creeps[c].Team != Me.Team && creeps[c].Health > 0 && creeps[c].Health <
                                Math.Floor(_spiderDmgStatick * (1 - creeps[c].DamageResist))
                ...
                if (countenemy >= 1)
                {
                    for (var t = 0; t < countenemy; ++t)
                    {
                        for (var s = 0; s < count; ++s)
                        {
                            ...
                            if (!(enemies[t].Position.Distance2D(list[s].Position) <= 800) ||
                                enemies[t].Team == Me.Team ||
                                enemies[t].Health <= 0 ||
                                !(enemies[t].Health < Math.Floor(_spiderDmg * (1 - enemies[t].DamageResist))) ||
        ...
        public void ChasingAll(EventArgs args)
        {
            ...
            if (E.IsAlive && !E.IsIllusion && Me.Distance2D(E) <= 1000)
            {
                ...
                {
                    if (Me.Distance2D(spinWeb) >= 900 && E.Distance2D(spinWeb) >= 900 && Me.Distance2D(E) <= 800 &&
                        Utils.SleepCheck(spinWeb.Handle + "SpideWeb"))
                    {
