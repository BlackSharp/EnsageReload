...
namespace DotaAllCombo.Heroes
{
    ...
    internal class TemplarAssassinController : Variables, IHeroController
    {
            ...
            if (Menu.Item("enabled").IsActive() && Utils.SleepCheck("combo"))
            {
                ...
                var v =
                    ObjectManager.GetEntities<Hero>()
                        .Where(
                            x => x.Team != Me.Team && x.IsAlive && x.IsVisible && !x.IsIllusion && !x.IsMagicImmune())
                        .ToList();
                ...
                if (Active && Me.Distance2D(E) <= 1400 && E != null && E.IsAlive && !Me.IsInvisible())
                {
                    ...
                    if ( // Dagon
                        Me.CanCast()
                        && _dagon != null
                        && (_ethereal == null || E.Modifiers.Any(y => y.Name == "modifier_item_ethereal_blade_slow") || _ethereal.Cooldown < 17)
                        && !E.IsLinkensProtected()
                        && _dagon.CanBeCasted()
                        && !meld
                        && !E.IsMagicImmune()
                    ...
                    {
                        _pike.UseAbility(E);
                        if ((_pike != null && _pike.CanBeCasted() || IsCasted(_pike)) && _r.CanBeCasted() &&
                            !Me.Modifiers.ToList().Exists(y => y.Name == "modifier_templar_assassin_meld") &&
                            Me.Distance2D(E.NetworkPosition) <= 400 && Me.CanCast() && !Me.IsSilenced() &&
                            !Me.IsHexed())
                        {
                    ...
                    var traps = ObjectManager.GetEntities<Unit>()
                        .Where(x => x.Name == "npc_dota_templar_assassin_psionic_trap" && x.Team == Me.Team
                                    && x.Distance2D(Me) <= 1700 && x.IsAlive && x.IsValid)
                        .ToList();
                    foreach (var q in traps)
                    {
                        if (!HurPikeActived() && E.NetworkPosition.Distance2D(q.Position) < 390 &&
                            q.Spellbook.SpellQ.CanBeCasted() && Utils.SleepCheck("traps") && !E.Modifiers.ToList()
                                .Exists(x => x.Name == "modifier_templar_assassin_trap_slow"))
                        {
        ...
        {
            return ability.Level > 0 && ability.CooldownLength > 0 && Math.Ceiling(ability.CooldownLength)
                       .Equals(Math.Ceiling(ability.Cooldown));
        }
