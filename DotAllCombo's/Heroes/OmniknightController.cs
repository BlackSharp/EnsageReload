...
#pragma
 warning disable CS0649 // Field 'OmniknightController.urn' is never assigned to, and will always have its default value null
        ...
        private Item _urn,
            ...
            _satanic,
#pragma
 warning restore CS0649 // Field 'OmniknightController.urn' is never assigned to, and will always have its default value null
            _medall,
            _glimmer,
            _manta,
            _pipe,
                ...
                for (var i = 0; i < countAlly; ++i)
                {
                    ...
                    for (var z = 0; z < countV; ++z)
                    {
                        ...
                        if (
                            ...
                            && v.Count(x => x.Distance2D(Me) <= _r.GetCastRange()) >=
                            Menu.Item("UltCountTarget").GetValue<Slider>().Value
                            && ally.Count(x => x.Distance2D(Me) <= _r.GetCastRange()) >=
                            Menu.Item("UltCountAlly").GetValue<Slider>().Value
                            && ally[i].Health <= ally[i].MaximumHealth / 100 *
                            Menu.Item("HealhUlt").GetValue<Slider>().Value
                        ...
                        if (
                            ...
                            && v.Count(x => x.Distance2D(Me) <= _guardian.GetCastRange()) >=
                            Menu.Item("healsetTarget").GetValue<Slider>().Value
                            && ally.Count(x => x.Distance2D(Me) <= _guardian.GetCastRange()) >=
                            Menu.Item("healsetAlly").GetValue<Slider>().Value
                            && ally[i].Health <= ally[i].MaximumHealth / 100 *
                            Menu.Item("HealhHeal").GetValue<Slider>().Value
                        ...
                        if (
                            ...
                            && v.Count(x => x.Distance2D(Me) <= _pipe.GetCastRange()) >=
                            Menu.Item("pipesetTarget").GetValue<Slider>().Value
                            && ally.Count(x => x.Distance2D(Me) <= _pipe.GetCastRange()) >=
                            Menu.Item("pipesetAlly").GetValue<Slider>().Value
                        ...
                        if (
                            _sphere != null && _sphere.CanBeCasted() &&
                            Me.Distance2D(ally[i]) <= _sphere.GetCastRange() + 50
                            && !ally[i].IsMagicImmune()
                            && (ally[i].Distance2D(v[z]) <= ally[i].AttackRange + ally[i].HullRadius + 10
                                || ally[i].Distance2D(v[z]) <= v[i].AttackRange + ally[i].HullRadius + 10
                        ...
                        if (
                            ...
                            Me.Distance2D(ally[i]) <= _glimmer.GetCastRange() + 50
                            && ally[i].Health <= Me.MaximumHealth * 0.5
        ...
        public void OnLoadEvent()
        {
            ...
            _ult.AddItem(
                new MenuItem("HealhUlt", "Min healh Ally % to Ult").SetValue(new Slider(35, 10, 70))); // x/ 10%
