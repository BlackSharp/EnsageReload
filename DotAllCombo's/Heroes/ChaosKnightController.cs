    ...
    internal class ChaosKnightController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            var a = ObjectManager.GetEntities<Hero>()
                .Where(x => x.Team == Me.Team && x.IsAlive && x.IsVisible && x.IsIllusion && x.IsControllable)
                .ToList();
            var illy = a.Count;
            if (illy >= 1)
            {
                for (var i = 0; i < illy; ++i)
                {
            ...
            if (!Active || !(Me.Distance2D(E) <= 1400) || E == null || !E.IsAlive || Toolset.InvUnit(Me)) return;
            {
                ...
                var pos = new Vector3((float) (E.Position.X - 70 * Math.Cos(angle)),
                    (float) (E.Position.Y - 70 * Math.Sin(angle)),
                    0);
                if (
                    _blink != null
                    && _q.CanBeCasted()
                ...
                if (
                    _w != null
                    && _w.CanBeCasted() && Me.Distance2D(E) <= _w.GetCastRange() + 300 && (Me.Distance2D(E) >= Me.GetAttackRange()
                                                                                           || Me.Distance2D(E) <= Me.GetAttackRange()
                                                                                           && E.NetworkActivity == NetworkActivity.Attack) && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_w.Name) && Utils.SleepCheck("W")
                )
                {
                    _w.UseAbility(E);
                    Utils.Sleep(200, "W");
