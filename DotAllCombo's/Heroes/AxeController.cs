...
namespace DotaAllCombo.Heroes
{
    ...
    internal class AxeController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            if (Active && Me.Distance2D(E) <= 2000 && E.IsAlive && !Me.IsInvisible())
            {
                ...
                if (_lotusorb != null && _lotusorb.CanBeCasted() &&
                    Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_lotusorb.Name) && v.Count(x => x.Distance2D(Me) <= 650) >= Menu.Item("Heelm").GetValue<Slider>().Value && Utils.SleepCheck("_lotusorb")
                )
                {
                ...
                if (_q != null && _q.IsInAbilityPhase &&
                    v.Count(x => x.Distance2D(Me) <= _q.GetCastRange() + Me.HullRadius + 23) == 0 &&
            ...
            if (Menu.Item("kill").IsActive())
            {
                ...
                for (var i = 0; i < count; ++i)
                {
                    ...
                    if (_r.IsInAbilityPhase &&
                        v.Where(x => Me.Distance2D(x) <= _r.GetCastRange() + Me.HullRadius + 24)
                            .OrderBy(z => z.Health)
                            .First()
                            .Health > _rDmg && v[i].Distance2D(Me) <= _r.GetCastRange() + Me.HullRadius + 24 &&
                    ...
                    if (_blink != null && _blink.CanBeCasted() && _r != null && _r.CanBeCasted() &&
                        Menu.Item("_blink").IsActive())
                    {
                        if (Me.Distance2D(v[i]) > _r.GetCastRange() + Me.HullRadius + 24 && v[i].Health < _rDmg &&
                            Utils.SleepCheck(v[i].Handle.ToString()))
                        {
                    ...
                    var bonusRange = Menu.Item("killRange").IsActive()
                        ? Menu.Item("Blade").GetValue<Slider>().Value
                        : 0;
                    if (v[i].Health <= _rDmg &&
                        v[i].Distance2D(Me) <= _r.GetCastRange() + Me.HullRadius + 24 + bonusRange &&
                        Utils.SleepCheck(v[i].Handle.ToString()))
                    {
                        _r.UseAbility(v[i]);
                        Utils.Sleep(150, v[i].Handle.ToString());
        ...
        public void OnLoadEvent()
        {
            ...
            Menu.AddItem(new MenuItem("Blade", "Bonus Ult Range").SetValue(new Slider(200, 1, 500)))
                .SetTooltip("Move to enemy");
        }
