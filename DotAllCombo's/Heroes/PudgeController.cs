...
namespace DotaAllCombo.Heroes
{
    ...
    internal class PudgeController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            {
                if (Me.HasModifier("modifier_pudge_rot") &&
                    v.Count(x => x.Distance2D(Me) <= W.GetCastRange() + Me.HullRadius) == 0)
                {
                    W.ToggleAbility();
                    Time.Start();
                }
                else if (!Me.HasModifier("modifier_pudge_rot") &&
                         v.Count(x => x.Distance2D(Me) <= W.GetCastRange() + Me.HullRadius) > 0)
                {
                    W.ToggleAbility();
                    Time.Start();
            ...
            if (Active)
            {
                ...
                EMoveSpeed = E.HasModifier("modifier_spirit_breaker_charge_of_darkness")
                    ? 550 + (int) E.Spellbook.Spell1.Level * 50
                    : E.MovementSpeed;

                var vector = new Vector2((float) Math.Cos(E.RotationRad) * EMoveSpeed,
                    (float) Math.Sin(E.RotationRad) * EMoveSpeed);
                var start = new Vector3(
                    (float) ((0.3 + Game.Ping / 1000) * Math.Cos(E.RotationRad) * EMoveSpeed + E.Position.X),
                    (float) ((0.3 + Game.Ping / 1000) * Math.Sin(E.RotationRad) * EMoveSpeed + E.NetworkPosition.Y),
                    E.NetworkPosition.Z);
                var specialPosition = new Vector3(
                    (float) (MinRangeHook * Math.Cos(E.RotationRad) + E.NetworkPosition.X),
                ...
                else if (Q.CanBeCasted() && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(Q.Name))
                {
                        ...
                        E.HasModifier("modifier_spirit_breaker_charge_of_darkness"))
                    {
                        for (var i = 0.03; i <= 0.135; i += 0.03)
                        {
                            var estimated = new Vector3(
                                (float) (i * Math.Cos(E.RotationRad) * EMoveSpeed + HookPosition.X),
                                (float) (i * Math.Sin(E.RotationRad) * EMoveSpeed + HookPosition.Y),
                                E.NetworkPosition.Z);
                        ...
                        CastPos = (HookPosition - Me.Position) *
                                  ((Q.GetCastRange() + Me.HullRadius) / HookPosition.Distance2D(Me.Position)) +
                                  Me.Position;
                    ...
                    {
                        CastPos = (specialPosition - Me.Position) *
                                  ((Q.GetCastRange() + Me.HullRadius) / specialPosition.Distance2D(Me.Position)) +
                                  Me.Position;
                ...
                else
                {
                    ...
                    if (Dagon != null && Dagon.CanBeCasted() && Me.Distance2D(E) <= Dagon.GetCastRange() &&
                    ...
                    if (Glimmer != null && Glimmer.CanBeCasted() && Me.Distance2D(E) <= 300 &&
                    ...
                    if (countElse == 11 && R != null && R.CanBeCasted() && Me.Distance2D(E) <= R.GetCastRange() + 150 &&
                    ...
                    if (countElse == 14 && Me.Distance2D(E) <= 300 && Mail != null
                        && Mail.CanBeCasted() && (E.NetworkActivity == NetworkActivity.Attack ||
                                                  E.Spellbook.Spells.All(x => x.IsInAbilityPhase))
                        && Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(Mail.Name)
                        && Utils.SleepCheck("mail"))
                    {
                        Mail.UseAbility();
                    ...
                    if ((R == null || !R.CanBeCasted() || !Q.CanBeCasted() &&
                         Me.Distance2D(E) >= R.GetCastRange() + Me.HullRadius && !E.HasModifier("pudge_meat_hook")) &&
                        !E.IsAttackImmune())
                    {
        ...
        public float GetTimeToTurn(Vector3 x)
        {
            ...
            return (float) ((Math.PI - Math.Abs(Math.Atan2(Math.Sin(m.RotationRad - angle),
                                 Math.Cos(m.RotationRad - angle)))) * (0.03 / 0.7));
        }
        ...
        public void CancelHook(object s, ElapsedEventArgs args)
        {
            ...
            var ePosition = new Vector3(
                (float) (travelTime * Math.Cos(E.RotationRad) * E.MovementSpeed + E.NetworkPosition.X),
                (float) (travelTime * Math.Sin(E.RotationRad) * E.MovementSpeed + E.NetworkPosition.Y), 0);
            if (E != null && E.NetworkActivity == NetworkActivity.Move && ePosition.Distance2D(HookPosition) >
