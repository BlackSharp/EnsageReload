        ...
        public void Combo()
        {
            ...
            if (Push && Me.IsAlive && _r != null && _r.CanBeCasted())
                if (!Me.HasModifier("modifier_bounty_hunter_wind_walk") || Me.IsVisibleToEnemies)
                    ...
                    {
                        var checkMod = v.Modifiers.Where(y => y.Name == "modifier_bounty_hunter_track")
                            .DefaultIfEmpty(null)
                            .FirstOrDefault();
                        var invItem = v.FindItem("item_glimmer_cape") ??
                                      v.FindItem("item_invis_sword") ??
                                      v.FindItem("item_silver_edge") ?? v.FindItem("item_glimmer_cape");
                        if (
                            (v.ClassId == ClassId.CDOTA_Unit_Hero_Riki ||
                             v.ClassId == ClassId.CDOTA_Unit_Hero_Broodmother
                             || v.ClassId == ClassId.CDOTA_Unit_Hero_Clinkz ||
                             v.ClassId == ClassId.CDOTA_Unit_Hero_Invoker
                             || v.ClassId == ClassId.CDOTA_Unit_Hero_SandKing ||
                             v.ClassId == ClassId.CDOTA_Unit_Hero_TemplarAssassin
                             || v.ClassId == ClassId.CDOTA_Unit_Hero_Treant ||
                             v.ClassId == ClassId.CDOTA_Unit_Hero_PhantomLancer
                             || v.Modifiers.Any(x =>
                            ...
                            && (!v.HasModifier("modifier_bounty_hunter_track") ||
                                checkMod != null && checkMod.RemainingTime <= 2)
                            && Utils.SleepCheck("R"))
                        {
                            _r.UseAbility(v);
                            Utils.Sleep(300, "R");
