...
namespace DotaAllCombo.Heroes
{
    ...
    internal class LegionCommanderController : Variables, IHeroController
    {
        ...
        private Unit GetLowestToQ(IEnumerable<Hero> units, Entity z)
        {
            ...
            {
                var creepsECount = ObjectManager.GetEntities<Unit>()
                    .Where(creep =>
                    ...
                    .ToList()
                    .Count();
                var enemiesCount = ObjectManager.GetEntities<Hero>()
                    .Where(x =>
                        x.Team != Me.Team && x.IsAlive && x.IsVisible && v.Distance2D(x) <= 330)
                    .ToList()
                    .Count();
                if (enemiesCount == 0)
                {
                ...
                if (v.HasModifier("modifier_shadow_demon_soul_catcher"))
                {
                    var demon =
                        ObjectManager.GetEntities<Hero>()
                            .FirstOrDefault(x => x.Team == Me.Team &&
                                                 x.ClassId == ClassId.CDOTA_Unit_Hero_Shadow_Demon);
                    if (demon != null)
        ...
        public void Combo()
        {
            ...
            if (Menu.Item("steal").IsActive() && !Me.HasModifier("modifier_legion_commander_duel") && _q != null &&
                _q.CanBeCasted())
            {
                if (!Me.IsAlive) return;
                var v =
                    ObjectManager.GetEntities<Hero>()
                        .Where(x => x.Team != Me.Team && x.IsAlive && x.Distance2D(Me) <= _q.GetCastRange() &&
                                    !x.IsIllusion)
                        .ToList();
                var dmg = GetLowestToQ(v, Me);

                if (dmg != null && Utils.SleepCheck("Q") &&
                    (Menu.Item("-dmg").IsActive() && !_r.CanBeCasted() || !Menu.Item("-dmg").IsActive()))
                {
            ...
            if (Active && !Game.IsChatOpen)
            {
                ...
                var pos = new Vector3((float) (E.Position.X - 55 * Math.Cos(angle)),
                    (float) (E.Position.Y - 55 * Math.Sin(angle)), 0);
                        ...
                        else
                        {
                            ...
                            if (E.IsLinkensProtected())
                            {
                                if ((_cyclone.CanBeCasted() || _force.CanBeCasted() || _halberd.CanBeCasted() ||
                                     _sheep.CanBeCasted() || _abyssal.CanBeCasted() || _atos.CanBeCasted()) &&
                                    Utils.SleepCheck("Combo2"))
                                {
                                    ...
                                    if (_satanic != null && _satanic.Cooldown <= 0 &&
                                        Me.Health <= Me.MaximumHealth * 0.5 &&
                            ...
                            else
                            {
                                if (UsedInvis(E))
                                {
                                    if (Me.Distance2D(E) <= 500)
                                    {
                                        ...
                                        else if (_sentry != null && _sentry.CanBeCasted() &&
                                                 Utils.SleepCheck("sentry") &&
                                                 _sentry != null &&
                                                 Menu.Item("Items3")
                                                     .GetValue<AbilityToggler>()
                ...
                if (!(Me.Distance2D(E) <= 1470) || !(Me.Distance2D(E) >= 350) || !Utils.SleepCheck("Move")) return;
                Me.Move(E.Position);
        ...
        private void DrawUltiDamage(EventArgs args)
        {
            ...
            if (!Menu.Item("dmg").IsActive()) return;
            _q = Me.Spellbook.SpellQ;
            int[] qDmg = {40, 80, 120, 160};
            int[] creepsDmg = {14, 16, 18, 20};
            int[] enemyDmg = {20, 40, 60, 80};
            ...
            foreach (var v in units.Where(x => !x.IsMagicImmune()))
            {
                ...
                var enemiesCount = ObjectManager.GetEntities<Hero>()
                    .Where(x =>
                        x.Team != Me.Team && x.IsAlive && x.IsVisible && v.Distance2D(x) <= 330 && !x.IsIllusion)
                    .ToList().Count;
                if (enemiesCount == 0)
                    enemiesCount = 0;
                if (creepsECount == 0)
                ...
                if (v.NetworkName == "CDOTA_Unit_Hero_Spectre" && v.Spellbook.Spell3.Level > 0)
                {
                    _damage =
                        Math.Floor(
                            (creepsECount * creepsDmg[_q.Level - 1] + enemiesCount * enemyDmg[_q.Level - 1] +
                             qDmg[_q.Level - 1]) *
                            (1 - (0.10 + v.Spellbook.Spell3.Level * 0.04)) * (1 - v.MagicDamageResist));
                }
        ...
        private static bool UsedInvis(Unit v)
        {
            return v.Modifiers.Any(
                x =>
        ...
        private static bool CanInvisCrit(Unit x)
        {
            return x.Modifiers.Any(
                m =>
        ...
        private void Drawing_OnEndScene(EventArgs args)
        {
            ...
            if (Menu.Item("Item").GetValue<AbilityToggler>().IsEnabled(_bkb.Name) &&
                Menu.Item("BKB").GetValue<KeyBind>().Active) return;
