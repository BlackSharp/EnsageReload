    ...
    internal class TinkerController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            var fount = ObjectManager.GetEntities<Unit>()
                ...
                .ToList();
            var creeps = ObjectManager.GetEntities<Creep>()
                .Where(creep =>
                    (creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane
                     || creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege
                     || creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral
                     || creep.ClassId == ClassId.CDOTA_BaseNPC_Creep) &&
                    creep.IsAlive && creep.Team != Me.Team && creep.IsVisible && creep.IsSpawned)
                .ToList();
            var panic = GetClosestToVector(TinkerCords.PanicPos, Me);
            var safe = GetClosestToVector(TinkerCords.SafePos, Me);
            ...
            if (Menu.Item("panicMod").IsActive() && Me.IsAlive)
            {
                    ...
                    && Me.Health <= Me.MaximumHealth / 100 * Menu.Item("Healh").GetValue<Slider>().Value)
                {
                    ...
                    else if (_r.IsChanneling || Me.HasModifier("modifier_tinker_rearm") || Me.IsChanneling()) return;
                    else if (
                        _r != null
                        && _r.CanBeCasted()
                        && Me.Distance2D(panic) <= HideAwayRange
            ...
            if (Active && !Game.IsChatOpen && Me.IsAlive)
            {
                ...
                if (E.IsAlive && !Me.IsChanneling())
                {
                    ...
                    else
                    {
                        ...
                        var pos = new Vector3((float) (E.Position.X - 550 * Math.Cos(angle)),
                            (float) (E.Position.Y - 550 * Math.Sin(angle)), 0);
                        uint elsecount = 0;
                        var v =
                            ObjectManager.GetEntities<Hero>()
                                .Where(x => x.Team != Me.Team && x.IsAlive && x.IsVisible && !x.IsIllusion)
                        ...
                        var magicimune = !E.IsMagicImmune() && !E.HasModifier("modifier_eul_cyclone");
                        if (Utils.SleepCheck("combo"))
                        {
                                ...
                                && magicimune && Utils.SleepCheck("Rearm"))
                            {
                                ...
                                if (Utils.SleepCheck("TimeEther") && Me.Distance2D(E) <= _ethereal.GetCastRange())
                                    Utils.Sleep(Me.NetworkPosition.Distance2D(E.NetworkPosition) / 620 * 1000,
                                        "TimeEther");
                            }
                            else
                                elsecount += 1;
                                ...
                                && magicimune && Utils.SleepCheck("Rearm"))
                            {
                                ...
                                if (Utils.SleepCheck("TimeW")
                                    ...
                                    Utils.Sleep(Me.NetworkPosition.Distance2D(E.NetworkPosition) / 600 * 1000,
                                        "TimeW");
                            }
                            else
                                elsecount += 1;
                ...
                if (E.IsAlive && !E.IsIllusion && !Me.IsChanneling() && Me.Distance2D(E) <= 2500)
                {
                    ...
                    if (Utils.SleepCheck("combo"))
                    {
                        ...
                        if (elsecount == 3
                            ...
                            && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_r.Name)
                            && !_r.IsChanneling
                            && (_soul != null && !_soul.CanBeCasted() || _soul == null || _w != null && !_w.CanBeCasted() || _w == null || (((_guardian != null && !_guardian.CanBeCasted())) || _guardian == null)
                            )
                            && Utils.SleepCheck("Rearm")
                        )
                        {
            ...
            if (Menu.Item("pushMod").IsActive() && !Active && !CastW && !CastE && Me.IsAlive)
            {
                ...
                {
                    if (_r.IsChanneling || Me.HasModifier("modifier_tinker_rearm") || Me.IsChanneling() ||
                        _r == null) return;
        ...
        {
            if (_ghost != null && _ghost.CanBeCasted() && Menu.Item("Item")
                    .GetValue<AbilityToggler>()
                    .IsEnabled(_ghost.Name)
                || _soul != null && _soul.CanBeCasted() &&
                Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_soul.Name)
                || _sheep != null && _sheep.CanBeCasted() &&
                Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_sheep.Name)
                || _q != null && _q.CanBeCasted() && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_q.Name)
                || _ethereal != null && _ethereal.CanBeCasted() &&
                Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_ethereal.Name)
                ...
                Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_guardian.Name)
                || _shiva != null && _shiva.CanBeCasted() &&
                Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_shiva.Name)
                || _eul != null && _eul.CanBeCasted() && Menu.Item("Item")
                    .GetValue<AbilityToggler>()
                    .IsEnabled(_eul.Name)
                || _glimmer != null && _glimmer.CanBeCasted() &&
                Menu.Item("Item").GetValue<AbilityToggler>().IsEnabled(_glimmer.Name))
                return false;
            return true;
        }
        ...
        public void OnLoadEvent()
        {
            ...
            Menu.AddItem(
                new MenuItem("keySpamW", "Use Missile and Mana items(DefMode)").SetValue(new KeyBind('F',
                    KeyBindType.Press)));
            Menu.AddItem(
                new MenuItem("keySpamE", "Use March and Mana items(DefMode)").SetValue(new KeyBind('G',
                    KeyBindType.Press)));
            ...
            _panics.AddItem(new MenuItem("panicMod", "Auto Blink and Travel base position if Healt <=| and have enemy")
                .SetValue(true));
            ...
            _panics.AddItem(new MenuItem("Panic Items", "Items").SetValue(new AbilityToggler(
        ...
        private void ParticleDraw(EventArgs args)
        {
            ...
            {
                for (var i = 0; i < TinkerCords.SafePos.Count(); ++i)
                {
                    if (!_iscreated)
                    {
                        var effect = new ParticleEffect(EffectPath, TinkerCords.SafePos[i]);
                ...
                {
                    for (var i = 0; i < TinkerCords.PanicPos.Count(); ++i)
                    {
                        if (!_iscreated)
                        {
                            var effect = new ParticleEffect(EffectPanicPath, TinkerCords.PanicPos[i]);
                            effect.SetControlPoint(1, new Vector3(HideAwayRange, 0, 0));
                            Effects.Add(effect);
                        }
                    }
