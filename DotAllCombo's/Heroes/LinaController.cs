...
namespace DotaAllCombo.Heroes
{
    ...
    internal class LinaController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            var eulModifier = E.Modifiers.Where(y => y.Name == "modifier_eul_cyclone")
                .DefaultIfEmpty(null)
                .FirstOrDefault();
            ...
            if (Me.IsAlive)
            {
                if (Active && E.IsVisible && Me.Distance2D(E) <= 2300 && !noBlade)
                {
                    ...
                    var start = E.NetworkActivity == NetworkActivity.Move
                        ? new Vector3(
                            (float) ((_w.GetCastDelay(Me, E, true) + 0.5) * Math.Cos(E.RotationRad) * E.MovementSpeed +
                                     E.Position.X),
                            (float) ((_w.GetCastDelay(Me, E, true) + 0.5) * Math.Sin(E.RotationRad) * E.MovementSpeed +
                                     E.NetworkPosition.Y), E.NetworkPosition.Z)
                        : E.NetworkPosition;
                    if (_w != null && _w.CanBeCasted()
                        ...
                        && (eulModifier != null && eulModifier.RemainingTime <= _w.GetCastDelay(Me, E, true) + 0.5
                            ...
                            && (_cyclone == null ||
                                !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_cyclone.Name) ||
                                _cyclone.Cooldown < 20 && _cyclone.Cooldown > 0)))
                    {
                        _w.UseAbility(start);
                        Utils.Sleep(150, "w");
                    }
                            ...
                            var chen =
                                ObjectManager.GetEntities<Hero>()
                                    .FirstOrDefault(x => x.Team == Me.Team &&
                                                         x.ClassId == ClassId.CDOTA_Unit_Hero_Chen);
                            if (chen != null)
                        ...
                        (Me.IsVisibleToEnemies || Game.IsKeyDown(Menu.Item("Combo Key").GetValue<KeyBind>().Key)))
                    {
                            ...
                            Utils.SleepCheck(v.Handle.ToString()))
                        {
                            ...
                            Utils.Sleep(500, v.Handle.ToString());
                                 ...
                                 Me.Distance2D(v) < _orchid.GetCastRange() &&
                                 ...
                                 Utils.SleepCheck(v.Handle.ToString()))
                        {
                            ...
                            Utils.Sleep(500, v.Handle.ToString());
                                 ...
                                 Me.Distance2D(v) < _atos.GetCastRange() - 400 &&
