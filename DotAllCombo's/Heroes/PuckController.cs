...
namespace DotaAllCombo.Heroes
{
    ...
    internal class PuckController : Variables, IHeroController
    {
            ...
            if (Game.IsKeyDown(Menu.Item("Full").GetValue<KeyBind>().Key) && Utils.SleepCheck("combo"))
            {
                    ...
                    !isInAdvantage)
                {
                    if (Me.CanCast())
                    {
                        ...
                        if (elsecount == 9 && _r != null && _r.CanBeCasted() &&
                ...
                Utils.SleepCheck("combo2"))
            {
                if (Me.CanCast())
                {
                    ...
                    if (elsecount == 9 && _e != null
                        && _e.CanBeCasted()
                        && _d != null
                        && _d.CanBeCasted() && ((_sheep == null || !_sheep.CanBeCasted() ||
                                                 !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_sheep.Name))
                                                && (_shiva == null || !_shiva.CanBeCasted() ||
                                                    !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_shiva.Name))
                                                && (_dagon == null || !_dagon.CanBeCasted() || !Menu.Item("Items")
                                                        .GetValue<AbilityToggler>()
                                                        .IsEnabled("item_dagon"))
                                                && (_ethereal == null || !_ethereal.CanBeCasted() || !Menu.Item("Items")
                                                        .GetValue<AbilityToggler>()
                                                        .IsEnabled(_ethereal.Name))
                                                && (_orchid == null || !_orchid.CanBeCasted() || !Menu.Item("Items")
                                                        .GetValue<AbilityToggler>()
                                                        .IsEnabled(_orchid.Name))
                                                && (_blink == null || !_blink.CanBeCasted() ||
                                                    !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_blink.Name))) && (_w == null || !_w.CanBeCasted() || Me.Distance2D(E) >= 400 || !Menu.Item("Skills")
                                                                                                                                    .GetValue<AbilityToggler>()
                                                                                                                                    .IsEnabled(_w.Name)) && Utils.SleepCheck("1"))
                    {
                        _e.UseAbility();
                        Utils.Sleep(250, "1");
                    }
                    if (
                        ...
                        && (_blink == null || !_blink.CanBeCasted() ||
                            !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_blink.Name))
                        && (_w == null || !_w.CanBeCasted() || Me.Distance2D(E) >= 400 || !Menu.Item("Skills")
                                .GetValue<AbilityToggler>()
                                .IsEnabled(_w.Name))
                        && Utils.SleepCheck("12"))
                    {
                        ...
                        if (baseDota != null)
                        {
                            for (var i = 0; i < baseDota.Count(); ++i)
                            {
        ...
        public void OnLoadEvent()
        {
            ...
            Menu.AddItem(
                new MenuItem("Full", "Full Combo(Please use only if you have blink)").SetValue(new KeyBind('A',
                    KeyBindType.Press)));
            Menu.AddItem(
                new MenuItem("Escape", "Escape Combo(Please use only if you have blink)").SetValue(new KeyBind('D',
                    KeyBindType.Press)));
