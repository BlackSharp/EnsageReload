...
namespace DotaAllCombo.Heroes
{
    ...
    internal class WeaverController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            {
                if (Me.Distance2D(E) <= 150 && Utils.SleepCheck("Attack") &&
                    Me.NetworkActivity != NetworkActivity.Attack)
                {
                    Me.Attack(E);
                    Utils.Sleep(150, "Attack");
                }
                else if (Me.Distance2D(E) <= 2400 && Me.Distance2D(E) >= 130 &&
                         Me.NetworkActivity != NetworkActivity.Attack && Utils.SleepCheck("Move"))
                {
                    Me.Move(E.Position);
        ...
        private void OnTimedEvent()
        {
            ...
            if (_r != null)
            {
                if (Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_r.Name))
                {
                    ...
                    float now = Me.Health;
                    Task.Delay(4000 - (int) Game.Ping)
                        .ContinueWith(_ =>
                        {
                            float back4 = Me.Health;
                            if ((Menu.Item("ultMode2").GetValue<bool>() && now - back4 >=
                                 Menu.Item("MomentDownHealth2").GetValue<Slider>().Value
                                 || Menu.Item("ultMode1").GetValue<bool>() &&
                                 (int) Me.MaximumHealth / (4 / (now - back4)) / 1000000 >=
                                 (double) Menu.Item("MomentDownHealth1").GetValue<Slider>().Value / 100) &&
                                _r.CanBeCasted())
                            {
                    ...
                    var ally = ObjectManager.GetEntities<Hero>()
                        .Where(x => x.Team == Me.Team && x.IsAlive && x.IsVisible && !x.IsIllusion && !x.Equals(Me))
                        .ToList();
                        ...
                        foreach (var a in ally)
                        {
                            ...
                            Task.Delay(4000 - (int) Game.Ping)
                                .ContinueWith(_ =>
                                {
                                    ...
                                    if ((Menu.Item("ultMode2Ally").GetValue<bool>() && allyHealth - backAlly4 >=
                                         Menu.Item("MomentAllyDownHealth2").GetValue<Slider>().Value
                                         ||
                                         Menu.Item("ultMode1Ally").GetValue<bool>() &&
                                         (int) a.MaximumHealth / (4 / (allyHealth - backAlly4)) / 1000000 >=
                                         (double) Menu.Item("MomentAllyDownHealth1")
                                             .GetValue<Slider>()
                                             .Value / 100) &&
                                        Me.Distance2D(a) <= 1000 + Me.HullRadius)
                                    {
        ...
        private void DrawQDamage(EventArgs args)
        {
            ...
            {
                float now = Me.Health;
                Task.Delay(4000 - (int) Game.Ping / 1000)
                    .ContinueWith(_ =>
                    {
                        float back4 = Me.Health;
                        _health = now - back4;
        ...
        public void OnLoadEvent()
        {
            ...
            _ultAlly.AddItem(new MenuItem("ult", "Use ult in Ally").SetValue(true))
                .SetTooltip("You need have AghanimS!");
            _ultMe.AddItem(new MenuItem("ultMode1", "Use 1 Mode(Number % of lost health)").SetValue(true));
            _ultMe.AddItem(new MenuItem("MomentDownHealth1", "Min Health % Down To Ult").SetValue(new Slider(35, 5)))
                .SetTooltip(
                    "Minimal damage % in my max health which I absorb values 4 seconds before using the Ultimate.");

            _ultMe.AddItem(new MenuItem("ultMode2", "Use 2 Mode(Number of lost health)").SetValue(true));
            _ultMe
                .AddItem(
                    new MenuItem("MomentDownHealth2", "Min Health Down To Ult").SetValue(new Slider(450, 200, 2000)))
                .SetTooltip("Minimal damage which I absorb values 4 seconds before using the Ultimate.");
            _ultAlly.AddItem(new MenuItem("ultMode1Ally", "Use 1 Mode(% Number of lost health)").SetValue(true));
            _ultAlly.AddItem(
                    new MenuItem("MomentAllyDownHealth1", "Min Health % Ally Down To Ult").SetValue(new Slider(35, 5)))
            ...
            _ultAlly
                .AddItem(
                    new MenuItem("MomentAllyDownHealth2", "Min Ally count Health Down To Ult").SetValue(new Slider(750,
                        300, 2000)))
                .SetTooltip("Damage which absorb ally in 4 seconds)");
