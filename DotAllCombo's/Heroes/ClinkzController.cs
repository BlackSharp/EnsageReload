namespace DotaAllCombo.Heroes
{
	...
	internal class ClinkzController : Variables, IHeroController
	{
	    ...
	    private readonly Menu orbwolk = new Menu("Orbwolk", "Orbwolk");
        public void OnLoadEvent()
	    {
	        AssemblyExtensions.InitAssembly("VickTheRock", "0.2b");
        ...
        public void Combo()
		{
            ...
            if (Active)
            {
                ...
                //Console.WriteLine(me.GetAttackRange() / 100 * Menu.Item("range").GetValue<Slider>().Value);

                if (Menu.Item("orbwalk").GetValue<bool>() && W !=null && W.CanBeCasted() && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(W.Name))
                {
                    var range = modifInv
                        ? me.Distance2D(e) <= me.GetAttackRange() / 100 * Menu.Item("range").GetValue<Slider>().Value
                        : me.Distance2D(e) >= me.GetAttackRange() / 100 * Menu.Item("range").GetValue<Slider>().Value;
				    ...
				    else if (!e.IsAttackImmune() && me.Distance2D(e) <= me.GetAttackRange() / 100 * Menu.Item("range").GetValue<Slider>().Value && !e.IsAttackImmune() && Utils.SleepCheck("attack"))
				    {
				        Orbwalking.Orbwalk(e, 0, 1600, true);
				    }
                }
				sheep = e.ClassId == ClassId.CDOTA_Unit_Hero_Tidehunter ? null : me.FindItem("item_sheepstick");
                
				if (me.Distance2D(e) <= 1400 && e.IsAlive && !modifInv)
				{
					if ( W == null || !W.CanBeCasted() || !Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(W.Name))
					{
					    if (me.Distance2D(e) > me.GetAttackRange() / 100 * Menu.Item("range").GetValue<Slider>().Value && !e.IsAttackImmune())
					    {
					        Orbwalking.Orbwalk(e, 0, 1600, false, true);
					    }
					    else if (me.Distance2D(e) < me.GetAttackRange() / 100 * Menu.Item("range").GetValue<Slider>().Value && !e.IsAttackImmune() && Utils.SleepCheck("attack"))
					    {
					        Orbwalking.Orbwalk(e, 0, 1600, false);
					    }
                    }
