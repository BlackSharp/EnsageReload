    ...
    internal class SandKingController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            if (Active && Utils.SleepCheck("Combo"))
            {
                ...
                var pos = new Vector3((float) (E.Position.X - (_q.GetCastRange() - 100) * Math.Cos(angle)),
                    (float) (E.Position.Y - (_q.GetCastRange() - 100) * Math.Sin(angle)), 0);
                uint elsecount = 1;
                if (elsecount == 1 && (_blink != null && _blink.CanBeCasted() && Me.Distance2D(pos) <= 1100 ||
                                       _blink == null && Me.Distance2D(E) <= _q.GetCastRange() - 50))
                {
                    if (
                ...
                if (Me.Distance2D(E) <= 2000 && E != null && E.IsAlive && !modifInv && !Me.IsChanneling() &&
                    (!_r.CanBeCasted() || !Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_r.Name)))
                {
