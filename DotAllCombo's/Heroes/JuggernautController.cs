...
namespace DotaAllCombo.Heroes
{
    ...
    internal class JuggernautController : Variables, IHeroController
    {
        ...
        public void Combo()
        {
            ...
            {
                var healingWard = ObjectManager.GetEntities<Unit>()
                    .Where(x => x.ClassId == ClassId.CDOTA_BaseNPC_Additive
                                && x.IsAlive && x.IsControllable && x.Team == Me.Team)
                    .ToList();
                if (healingWard.Count >= 1)
                {
                    for (var i = 0; i < healingWard.Count(); ++i)
                    {
                        if (Me.Position.Distance2D(healingWard[i].Position) > 5 &&
                            Utils.SleepCheck(healingWard[i].Handle.ToString()))
                        {
            ...
            if (Active && Me.Distance2D(E) <= 1400)
            {
                ...
                if (_r != null && _r.CanBeCasted() && Me.Distance2D(E) <= 600 &&
                    Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_r.Name))
                {
                    var creep = ObjectManager.GetEntities<Creep>()
                        .Where(x => x.IsAlive && x.Team != Me.Team && x.IsSpawned)
                        .ToList();
