...
namespace DotaAllCombo.Heroes
{
    ...
    internal class FacelessVoidController : Variables, IHeroController
    {
        ...
        public void OnLoadEvent()
        {
            ...
            Menu.AddItem(new MenuItem("time_dilation", "Min targets to TimeDilation").SetValue(new Slider(2, 1, 5)))
                .SetTooltip("TODO UPDATE LOGIC");
            ...
            _ult.AddItem(
                new MenuItem("MomentDownHealth", "Min Health Down To Ult").SetValue(new Slider(450, 200, 2000)));
        ...
        public void Combo()
        {
            ...
            if (Active && Me.Distance2D(E) <= 1400 && E.IsAlive && !Me.IsInvisible())
            {
                ...
                if (
                    ...
                    && (_r == null || !_r.CanBeCasted() ||
                        ...
                        v.Count(x => x.Distance2D(Me) <= 425 + Me.HullRadius)
            ...
            if (Active)
            {
                ...
                for (var i = 0; i < v.Count; ++i)
                {
                    ...
                    {
                        if (_blink != null && _blink.CanBeCasted() && Me.Distance2D(v[i]) <= _blink.GetCastRange() &&
                            Me.Distance2D(v[i]) > _r.GetCastRange() + Me.HullRadius
                            && Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_blink.Name) &&
                            Utils.SleepCheck("blink"))
                        {
                        ...
                        if (_q != null && _q.CanBeCasted() &&
                            Me.Distance2D(v[i]) <= _q.GetCastRange() + Me.HullRadius &&
                            Me.Distance2D(v[i]) > _r.GetCastRange() + Me.HullRadius
                            && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_q.Name) &&
                            Utils.SleepCheck("Q"))
                        {
