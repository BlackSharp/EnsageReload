...
#pragma
 warning disable CS0649 // Field 'MeepoController.Travel' is never assigned to, and will always have its default value null
        public Item Blink, Travel, Shiva, Sheep, Medall, Dagon, Cheese, Ethereal, Vail, Atos, Orchid, Abyssal;
#pragma
 warning restore CS0649 // Field 'MeepoController.Travel' is never assigned to, and will always have its default value null
        public Font Txt;
        public Font Not;
#pragma
 warning disable CS0649 // Field 'MeepoController.meepos' is never assigned to, and will always have its default value null
        public List<Hero> Meepos;
#pragma
 warning restore CS0649 // Field 'MeepoController.meepos' is never assigned to, and will always have its default value null
        ...
        public void Combo()
        {
            ...
            var checkObj = ObjectManager.GetEntities<Unit>()
                .Where(x => (x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane
                             || x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege
                             || x.ClassId == ClassId.CDOTA_BaseNPC_Creep
                             || x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral
                             || x.HasInventory
                             || x.ClassId == ClassId.CDOTA_Unit_SpiritBear) && x.IsAlive && x.Team != Me.Team &&
                            x.IsValid)
                .ToList();
            var meepos = ObjectManager.GetEntities<Hero>()
                .Where(x => x.IsControllable && x.IsAlive && x.ClassId == ClassId.CDOTA_Unit_Hero_Meepo)
                .ToList();


            var fount = ObjectManager.GetEntities<Unit>()
                .Where(x => x.Team == Me.Team && x.ClassId == ClassId.CDOTA_Unit_Fountain)
                .ToList();
            ...
            var f = ObjectManager.GetEntities<Hero>()
                .Where(x => x.IsAlive && x.Team == Me.Team && !x.IsIllusion && x.IsControllable &&
            ...
            if (Dodge && Me.IsAlive)
            {
                ...
                if (baseDota != null)
                {
                    for (var t = 0; t < baseDota.Count(); ++t)
                    {
                        for (var i = 0; i < meeposCount; ++i)
                        {
                            ...
                            var pos = new Vector3((float) (baseDota[t].Position.X - 710 * Math.Cos(angle)),
                                (float) (baseDota[t].Position.Y - 710 * Math.Sin(angle)), 0);
                ...
                var thinker =
                    ObjectManager.GetEntities<Unit>()
                        .Where(unit => unit.Name == "npc_dota_thinker" && unit.Team != Me.Team)
                        .ToList();
                if (thinker != null)
                {
                    for (var i = 0; i < thinker.Count(); ++i)
                    {
                        for (var j = 0; j < meeposCount; ++j)
                        {
                            ...
                            var pos = new Vector3((float) (thinker[i].Position.X - 360 * Math.Cos(angle)),
                                (float) (thinker[i].Position.Y - 360 * Math.Sin(angle)), 0);
                ...
                {
                    if (Utils.SleepCheck(v.Handle + "_move") && v.Health <=
                    ...
                    var pos = new Vector3((float) (fount.First().Position.X - 500 * Math.Cos(angle)),
                        (float) (fount.First().Position.Y - 500 * Math.Sin(angle)), 0);
                ...
                for (var i = 0; i < meeposCount; ++i)
                {
                    ...
                    else if (!q[i].CanBeCasted() && meepos[i].Health <=
                             meepos[i].MaximumHealth / 100 * Menu.Item("healh").GetValue<Slider>().Value)
                    {
                        ...
                        for (var j = 0; j < meeposCount; ++j)
                        {
                            ...
                            q[j].CastSkillShot(E);
                            Utils.Sleep(q[j].GetCastDelay(meepos[j], E, true) + 1500,
                                meepos[i].Handle + "_net_casting");
                            break;
                        }
                    }
                    if (E != null
                        ...
                        && (q == null || !q[i].CanBeCasted() || E.HasModifier("modifier_meepo_earthbind") || !E.IsMagicImmune() || meepos[i].Distance2D(E) >= 1000)
            ...
            if (PoofAutoMode)
            {
                for (var i = 0; i < meeposCount; i++)
                {
                    var nCreeps = ObjectManager.GetEntities<Unit>()
                        .Where(x => (x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane
                                     || x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege
                                     || x.ClassId == ClassId.CDOTA_BaseNPC_Creep
                                     || x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral) && x.Team != Me.Team &&
                                    x.IsSpawned && x.IsAlive)
                        .Where(x => x.Distance2D(meepos[i]) <= 345)
                        .ToList()
                        .Count();
            ...
            if (!Activated) return;
            {
                for (var i = 0; i < meeposCount; ++i)
                {
                    ...
                    )
                    {
                        ...
                        {
                            q[i].CastSkillShot(E);
                            Utils.Sleep(q[i].GetCastDelay(meepos[i], E, true) + 1500,
                                meepos[i].Handle + "_net_casting");
                            Utils.Sleep(1500, "Q");
                        }
                    }
                    ...
                    )
                    {
                        if (Blink.CanBeCasted()
                            ...
                            && meepos[i].Health >= meepos[i].MaximumHealth / 100 *
                        ...
                        for (var j = 0; j < meeposCount; ++j)
                        {
                            if (w[j] == null || meepos[j].Handle == Me.Handle || !meepos[j].CanCast() ||
                                ...
                                Menu.Item("healh").GetValue<Slider>().Value || E.IsMagicImmune() ||
                                !w[j].CanBeCasted() ||
                    ...
                    else if ((!meepos[i].CanAttack()
                              || meepos[i].Distance2D(E) >= 0)
                             && meepos[i].NetworkActivity != NetworkActivity.Attack
                             && meepos[i].Distance2D(E) <= 1000
                             && (meepos[i].Handle != Me.Handle && Blink != null && Blink.CanBeCasted() && Me.Distance2D(E) <= 350
                ...
                if ( // Dagon
                    Me.CanCast()
                    && Dagon != null
                    && (Ethereal == null || E.HasModifier("modifier_item_ethereal_blade_slow") || Ethereal.Cooldown < 17)
                    && !E.IsLinkensProtected()
                    && Dagon.CanBeCasted()
                    && !E.IsMagicImmune()
        ...
        public void OnLoadEvent()
        {
            ...
            Print.LogMessage.Success(
                "<font face='verdana' color='#f80000'>Probably gonna dig a grave or two before this is done.</font>");
        ...
        public Hero ClosestToMouse(Hero source, float range = 90000)
        {
            ...
            foreach (var enemyHero in enemyHeroes.Where(
                enemyHero => closestHero[0] == null || closestHero[0].Distance2D(mousePosition) >
                             enemyHero.Distance2D(mousePosition)))
            {
