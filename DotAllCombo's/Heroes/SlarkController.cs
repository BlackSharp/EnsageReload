    ...
    internal class SlarkController : Variables, IHeroController
    {
        ...
        public async void Combo()
        {
            ...
            if (!Active) return;
            {
                ...
                var pos = new Vector3((float) (E.Position.X - 100 * Math.Cos(angle)),
                    (float) (E.Position.Y - 100 * Math.Sin(angle)), 0);
                    ...
                    && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_w.Name))
                {
                    ...
                    {
                        var vectorOfMovement = new Vector2((float) Math.Cos(E.RotationRad) * E.MovementSpeed,
                            (float) Math.Sin(E.RotationRad) * E.MovementSpeed);
                        var hitPosition = Interception(E.Position, vectorOfMovement, Me.Position, 933.33f);
                        var hitPosMod = hitPosition + new Vector3(vectorOfMovement.X * TimeToTurn(Me, hitPosition),
                                            vectorOfMovement.Y * TimeToTurn(Me, hitPosition), 0);
                        var hitPosMod2 = hitPosition +
                                         new Vector3(vectorOfMovement.X * TimeToTurn(Me, hitPosMod),
                                             vectorOfMovement.Y * TimeToTurn(Me, hitPosMod), 0);
                        ...
                        {
                            Me.Move((hitPosMod2 - Me.Position) * 50 / (float) GetDistance2D(hitPosMod2, Me) +
                                    Me.Position);
                        }
                    }
                    else
        ...
        {
            if (!(a is Unit || a is Vector3))
                throw new ArgumentException("Not valid parameters, accepts Unit|Vector3 only", nameof(a));
            if (!(b is Unit || b is Vector3))
                throw new ArgumentException("Not valid parameters, accepts Unit|Vector3 only", nameof(b));
        ...
        {
            if (!(enemy is Unit || enemy is Vector3))
                throw new ArgumentException("TimeToTurn => INVALID PARAMETERS!", nameof(enemy));
        ...
        private static float TimeToTurn(Entity startUnit, dynamic enemy)
        {
            ...
            const double turnRate =
                0.5; //Game.FindKeyValues(string.Format("{0}/MovementTurnRate", StartUnit.Name), KeyValueSource.Hero).FloatValue; // (Only works in lobby)
