...
namespace DotaAllCombo.Heroes
{
    ...
    internal class EarthSpiritController : Variables, IHeroController
    {
        public void Combo()
        {
            ...
            var charge =
                Me.Modifiers.FirstOrDefault(y => y.Name == "modifier_earth_spirit_stone_caller_charge_counter");

            var remnant = ObjectManager.GetEntities<Unit>()
                .Where(x => x.ClassId == ClassId.CDOTA_Unit_Earth_Spirit_Stone && x.Team == Me.Team && x.IsValid)
                .ToList();
        ...
        private void Others(EventArgs args)
        {
            ...
            var magnetizemod = E.Modifiers.Where(y => y.Name == "modifier_earth_spirit_magnetize")
                .DefaultIfEmpty(null)
                .FirstOrDefault();

            if (_autoUlt && magnetizemod != null && magnetizemod.RemainingTime <= 0.2 + Game.Ping &&
                Me.Distance2D(E) <= _d.GetCastRange() && Utils.SleepCheck("Rem"))
            {
                _d.UseAbility(E.Position);
                Utils.Sleep(1000, "Rem");
            }
            var remnant = ObjectManager.GetEntities<Unit>()
                .Where(x => x.ClassId == ClassId.CDOTA_Unit_Earth_Spirit_Stone && x.Team == Me.Team
            ...
            if (CastE && Me.Distance2D(E) <= 1400 && E != null && E.IsAlive && !Me.IsInvisible())
            {
                ...
                if (remnant.Count(x => x.Distance2D(Me) <= 1200) >= 1)
                {
                    for (var i = 0; i < remnantCount; ++i)
                    {
                        ...
                        {
                            if (
                                _d.CanBeCasted() && _e != null && _e.CanBeCasted() && !r.HasModifier("modifier_earth_spirit_boulder_smash") && !r.HasModifier("modifier_earth_spirit_geomagnetic_grip")
                            )
                            {
                                if (Me.Distance2D(E) <= _e.GetCastRange() - 50
                                    && Utils.SleepCheck("Rem"))
