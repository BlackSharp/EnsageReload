                ...
                else
                {
                    switch (targetMode)
                    {
                        case 0:
                            ...
                            {
                                case 0:
                                    var enemyHero0 = ObjectManager.GetEntities<Hero>()
                                        .Where(enemy => enemy.Team == _me.GetEnemyTeam() && enemy.IsAlive &&
                                                        !enemy.IsIllusion &&
                                                        enemy.Distance2D(Game.MousePosition) <= targetFindRange)
                                        .ToList();
                        ...
                        case 1:
                            switch (targetFindSource)
                            {
                                case 0:
                                    ...
                                    break;
                                case 1:
                                    var enemyHero1 = ObjectManager.GetEntities<Hero>()
                                        .Where(enemy => enemy.Team == _me.GetEnemyTeam() && enemy.IsAlive &&
                                                        !enemy.IsIllusion &&
                                                        enemy.Distance2D(_me.Position) <= targetFindRange)
                                        .ToList();
            ...
            else
            {
                switch (targetMode)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        var enemyHero = ObjectManager.GetEntities<Hero>()
                            .Where(enemy => enemy.Team == _me.GetEnemyTeam() && enemy.IsAlive && !enemy.IsIllusion &&
                                            enemy.Distance2D(Game.MousePosition) <= targetFindRange)
                            .ToList();
                ...
                if (_me.IsAlive)
                {
                    ...
                    for (var i = 0; i < count; ++i)
                    {
                        var v = ObjectManager.GetEntities<Hero>()
                            .Where(x => x.Team == _me.Team && x.IsAlive && x.IsVisible && !x.IsIllusion)
                            .ToList();
                        if (units[i].Name == "npc_dota_juggernaut_healing_ward")

                        {
                            if (_me.Position.Distance2D(units[i].Position) > 5 &&
                        ...
                        else if (units[i].Name == "npc_dota_neutral_ogre_magi")
                        {
                            for (var z = 0; z < v.Count(); ++z)
                            {
                                ...
                                if ((!v[z].HasModifier("modifier_ogre_magi_frost_armor") ||
                                     !_me.HasModifier("modifier_ogre_magi_frost_armor")) && armor.CanBeCasted() &&
                                    units[i].Position.Distance2D(v[z]) <= 900
                                    && Utils.SleepCheck(units[i].Handle.ToString()))
                                {
                        ...
                        if (_e.IsAlive && !_e.IsInvul() && (holdKey || toggleKey))
                        {
                            ...
                            {
                                if (_e.Position.Distance2D(units[i].Position) < 550 &&
                                    (!checkSetka || !checkStun || !_e.IsHexed() || !_e.IsStunned()) &&
                            ...
                            else if (units[i].Name == "npc_dota_neutral_big_thunder_lizard")
                            {
                                ...
                                if (_e.Position.Distance2D(units[i].Position) < 550 &&
                                    units[i].Spellbook.SpellW.CanBeCasted() &&
                                    Utils.SleepCheck(units[i].Handle + "lizard"))
                                {
                            ...
                            {
                                if (_e.Position.Distance2D(units[i].Position) < 200 &&
                                    (!checkSetka || !checkStun || !_e.IsHexed() || !_e.IsStunned()) &&
                                    units[i].Spellbook.SpellQ.CanBeCasted() &&
                            ...
                            else if (units[i].Name == "npc_dota_neutral_big_thunder_lizard")
                            {
                                ...
                                for (var z = 0; z < v.Count(); z++)
                                {
                                    if (!units[i].Spellbook.SpellW.CanBeCasted() ||
                                        !(units[i].Position.Distance2D(v[z]) <= 900)) continue;
                                    if (!(_e.Position.Distance2D(v[z]) < v[z].AttackRange + 150) ||
                                        !Utils.SleepCheck(units[i].Handle + "lizard")) continue;
                            ...
                            {
                                if (_e.Position.Distance2D(units[i].Position) < 240 &&
                                    units[i].Spellbook.SpellQ.CanBeCasted() &&
                            ...
                            {
                                var damageModif = units[i]
                                    .Modifiers
                                    .FirstOrDefault(x => x.Name == "modifier_visage_summon_familiars_damage_charge");

                                if (_e.Position.Distance2D(units[i].Position) < 1550 && units[i].Health < 6 &&
                                ...
                                if (_e.Position.Distance2D(units[i].Position) < 340 && damageModif.StackCount < 1 && !_e.IsStunned() && units[i].Spellbook.Spell1.CanBeCasted() && Utils.SleepCheck(units[i].Handle.ToString()))
                                {
                                    units[i].Spellbook.Spell1.UseAbility();
                            ...
                            {
                                if (!_me.AghanimState() && _me.Position.Distance2D(units[i]) <= 1200 ||
                                    _me.AghanimState())
                                {
                                    ...
                                    _medall = units[i].FindItem("item_medallion_of_courage") ??
                                              units[i].FindItem("item_solar_crest");


                                    if (_boots != null && _e.Position.Distance2D(units[i].Position) < 1550 &&
                                        _boots.CanBeCasted() &&
                                        Utils.SleepCheck(units[i].Handle.ToString()))
                                    {
                                        _boots.UseAbility();
                                    ...
                                    if (_mjollnir != null && _e.Position.Distance2D(units[i].Position) < 525 &&
                                    ...
                                    if (_medall != null && _e.Position.Distance2D(units[i].Position) < 525 &&
                                        _medall.CanBeCasted() &&
                                        Utils.SleepCheck(units[i].Handle.ToString()))
                                    {
                                    ...
                                    if (_mom != null && _e.Position.Distance2D(units[i].Position) < 525 &&
                                        _mom.CanBeCasted() &&
                                        Utils.SleepCheck(units[i].Handle.ToString()))
                                    {
                                        _mom.UseAbility();
                                    ...
                                    if (_abyssal != null && _e.Position.Distance2D(units[i].Position) < 500 &&
                                    ...
                                    if (_midas != null)
                                    {
                                        var neutrals = ObjectManager.GetEntities<Creep>()
                                            .Where(creep => (creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane ||
                                                             creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege ||
                                                             creep.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral ||
                                                             creep.ClassId == ClassId
                                                                 .CDOTA_BaseNPC_Invoker_Forged_Spirit ||
                                                             creep.ClassId == ClassId.CDOTA_BaseNPC_Creep) &&
                                                            creep.IsAlive && creep.IsVisible && creep.IsSpawned &&
                                                            creep.Distance2D(units[i]) <= 700 && creep.Team != _me.Team)
                                            .OrderBy(x => x.Health)
                                            .LastOrDefault();
                            ...
                            if (units[i].Distance2D(_e) <= units[i].AttackRange + 100 && !_e.IsAttackImmune()
                                && !units[i].IsAttacking() && units[i].CanAttack() &&
                                Utils.SleepCheck(units[i].Handle + "Attack")
                            )
                            {
                                units[i].Attack(_e);
                                Utils.Sleep(250, units[i].Handle + "Attack");
                                     ...
                                     && units[i].CanMove() && units[i].Distance2D(_e) <= 4000 &&
                                     Utils.SleepCheck(units[i].Handle + "Move")
                            )
                            {
                                ...
                                Utils.Sleep(350, units[i].Handle + "Move");
                                     ...
                                     && units[i].CanMove() && units[i].Distance2D(_e) <= 4000 &&
                                     Utils.SleepCheck(units[i].Handle + "MoveAttack")
                            )
                            {
                                units[i].Move(_e.Predict(300));
