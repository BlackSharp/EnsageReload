...
using SharpDX;
//VickTheRock лучший человек на этой планете.
namespace TinyReload
{
    internal class Program
    {
        ...
        private static void OnLoadEvent(object sender, EventArgs args)
        {
            ...
            Menu.AddItem(new MenuItem("first", "Forced checking that the enemy hero was the first unit.").SetValue(true)
                .SetTooltip("Tiny will not use Toss if the enemy hero is not the nearest unit."));
            Menu.AddToMainMenu();
        ...
        private static double GetDistance2D(Vector3 a, Vector3 b)
        {
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (!Game.IsInGame || Game.IsWatchingGame || _me == null ||
                _me.ClassId != ClassId.CDOTA_Unit_Hero_Tiny) return;
            ...
            _vail = _e.FindItem("item_veil_of_discord");
            ...
            if (_e != null)
            {
                ObjectManager.GetEntities<Unit>()
                    .Where(x => x.Team != _me.Team && (x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane ||
                                                       x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege ||
                                                       x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral ||
                                                       ReferenceEquals(x, _e)))
                    .ToList();
                var tinny = ObjectManager.GetEntities<Hero>()
                    .Where(x => x.Team == _me.Team && x.Name == _me.Name)
                    .ToList();

                var enem = ObjectManager.GetEntities<Unit>()
                    .Where(x => x.IsAlive && x.Distance2D(_me) <= _w.CastRange + _me.HullRadius &&
                                x.Handle != _me.Handle && (x.HasInventory ||
                                                           x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane ||
                                                           x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege ||
                                                           x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral))
                    .OrderBy(x =>
                    {
                        var firstOrDefault = tinny.OrderBy(y => GetDistance2D(x.Position, y.Position)).FirstOrDefault();
            ...
            if (_active)
                if (Menu.Item("orbwalk").GetValue<bool>() && _me.Distance2D(_e) <= 1600)
                    Orbwalking.Orbwalk(_e, 0, 1600, true, true);
            if (!_active || !(_me.Distance2D(_e) <= 1400) || !_e.IsAlive || isInvisible) return;
            {
                var angle = _me.FindAngleBetween(_e.Position, true);
                var pos = new Vector3((float) (_e.Position.X - 100 * Math.Cos(angle)),
                    (float) (_e.Position.Y - 100 * Math.Sin(angle)), 0);
                if (
                    _blink != null
                    && _q.CanBeCasted()
                    && _me.CanCast()
                ...
                if (_w != null && _w.CanBeCasted() && _me.Distance2D(_e) <= 275 + _w.CastRange
                    && Menu.Item("Skills").GetValue<AbilityToggler>().IsEnabled(_w.Name) && Utils.SleepCheck("W"))
                {
                    if (Menu.Item("first").IsActive())
                    {
                        var tinny = ObjectManager.GetEntities<Hero>()
                            .Where(x => x.Team == _me.Team && x.Name == _me.Name)
                            .ToList();
                        ...
                        var enem = ObjectManager.GetEntities<Unit>()
                            .Where(x => x.IsAlive && x.Distance2D(_me) <= _w.CastRange + _me.HullRadius &&
                                        x.Handle != _me.Handle &&
                                        (x.HasInventory || x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Lane ||
                                         x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Siege ||
                                         x.ClassId == ClassId.CDOTA_BaseNPC_Creep_Neutral))
                            .OrderBy(x =>
                            {
                                var firstOrDefault = tinny.OrderBy(y => GetDistance2D(x.Position, y.Position))
                                    .FirstOrDefault();
                                if (firstOrDefault != null)
                                    return GetDistance2D(x.Position,
                                        firstOrDefault.Position);
                ...
                if (_bkb == null || !_bkb.CanBeCasted() ||
                    v.Count(x => x.Distance2D(_me) <= 650) < Menu.Item("Heel").GetValue<Slider>().Value ||
                    !Menu.Item("Items").GetValue<AbilityToggler>().IsEnabled(_bkb.Name) ||
                    !Utils.SleepCheck("bkb")) return;
                _bkb.UseAbility();
                Utils.Sleep(100, "bkb");
            }
        }
